  ...
  if(rid == root_rid )
    {
      /* This block will only run on the node that matches root_rid .*/
        for (int i0 = 0; i0 < m0; ++i0) {
            ...
            for (int p0 = 0; p0 < loopEnd; ++p0) {
                res1 += input_distributed[p0 + i0 + loopEnd] * weights_distributed[p0 + loopEnd];
            }

            for (int p0 = loopEnd; p0 < m0; ++p0) {
                res2 += input_distributed[p0 + i0 +  loopEnd - k0] * weights_distributed[p0 + loopEnd - k0];
            }

            output_distributed[i0] = res1 + res2;
        }
