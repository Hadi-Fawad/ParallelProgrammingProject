                   ...
                   float *output_distributed )
{
    ...
    if(rid == root_rid)
    {
        // Initialize output_distributed to 0.0f
        ...
        for (int p0 = 0; p0 < k0; ++p0)
        {
            for (int i0 = 0; i0 < m0; ++i0)
            {
                int index = (p0 + i0) % m0; // Keeping modulo for wrapping around
                output_distributed[i0] += input_distributed[index] * weights_distributed[p0];
            }
        }
    }
    else
    {
        // Code for other ranks if needed
    }
}
